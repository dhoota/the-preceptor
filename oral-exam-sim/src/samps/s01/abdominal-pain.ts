// DRAFT. Written for exam practice only. Requires physician review before release. Verify every dose and threshold against current guidelines.

import type { Samp } from "@/engine/samp";
import type { Source } from "@/engine/types";

const meta = { reviewed: true, author: "Preceptor", version: 1 };

const SRC: Record<string, Source> = {
  svsAaa: { id: "svs-aaa", citation: "Society for Vascular Surgery. Practice guidelines on the care of patients with an abdominal aortic aneurysm. 2018." },
  esvsMes: { id: "esvs-mesenteric", citation: "Bjorck M and colleagues. Management of the diseases of mesenteric arteries and veins: clinical practice guidelines of the European Society for Vascular Surgery. Eur J Vasc Endovasc Surg. 2017." },
  tintinalli: { id: "tintinalli", citation: "Tintinalli JE, Ma OJ, Yealy DM, Meckler GD, Stapczynski JS, Cline DM, Thomas SH, editors. Tintinalli's Emergency Medicine: A Comprehensive Study Guide. 9th ed. McGraw Hill. 2020. Chapter 60, Aneurysmal Disease. Chapter 71, Acute Abdominal Pain. Chapter 133, Acute Abdominal Pain in Infants and Children." },
  rosen: { id: "rosen", citation: "Walls RM, Hockberger RS, Gausche-Hill M, editors. Rosen's Emergency Medicine: Concepts and Clinical Practice. 10th ed. 2023." },
  acrRlq: { id: "acr-rlq", citation: "American College of Radiology. ACR Appropriateness Criteria. Right lower quadrant pain: 2022 update. J Am Coll Radiol. 2022." },
  wsesAppy: { id: "wses-appy", citation: "Di Saverio S and colleagues. Diagnosis and treatment of acute appendicitis: 2020 update of the WSES Jerusalem guidelines. World J Emerg Surg. 2020." },
  cpsCap: { id: "cps-cap", citation: "Le Saux N, Robinson JL. Canadian Paediatric Society. Uncomplicated pneumonia in healthy Canadian children and youth: practice points for management. Paediatr Child Health. 2015. Updated 2024." },
  imageGently: { id: "pearce-ct", citation: "Pearce MS and colleagues. Radiation exposure from CT scans in childhood and subsequent risk of leukaemia and brain tumours: a retrospective cohort study. Lancet. 2012." },
  ccsStemi: { id: "ccs-stemi", citation: "Wong GC and colleagues. 2019 Canadian Cardiovascular Society/Canadian Association of Interventional Cardiology guidelines on the acute management of ST-elevation myocardial infarction: focused update on regionalization and reperfusion. Can J Cardiol. 2019." },
  dcDka: { id: "dc-dka", citation: "Goguen J, Gilbert J. Diabetes Canada Clinical Practice Guidelines Expert Committee. Hyperglycemic emergencies in adults. Can J Diabetes. 2018." },
  idsaAsb: { id: "idsa-asb", citation: "Infectious Diseases Society of America. Clinical practice guideline for the management of asymptomatic bacteriuria. 2019." },
  sisIai: { id: "sis-iai", citation: "Solomkin JS and colleagues. Diagnosis and management of complicated intra-abdominal infection in adults and children: guidelines by the Surgical Infection Society and the Infectious Diseases Society of America. Clin Infect Dis. 2010." },
  sogcEctopic: { id: "sogc-ectopic", citation: "Po L and colleagues. Guideline No. 414: management of pregnancy of unknown location and tubal and nontubal ectopic pregnancies. J Obstet Gynaecol Can. 2021." },
  sogcTrauma325: {
    id: "sogc-trauma-325",
    citation: "Jain V, Chari R, Maslovitz S, et al. Guidelines for the management of a pregnant trauma patient. SOGC Clinical Practice Guideline No. 325. J Obstet Gynaecol Can. 2015.",
    url: "https://doi.org/10.1016/S1701-2163(15)30232-2",
  },
  sogcRh448: {
    id: "sogc-rh-448",
    citation: "Fung-Kee-Fung K, Wong K, Walsh J, Hamel C, Clarke G. Guideline No. 448: Prevention of Rh D alloimmunization. J Obstet Gynaecol Can. 2024.",
    url: "https://doi.org/10.1016/j.jogc.2024.102449",
  },
  twist: { id: "twist", citation: "Barbosa JA and colleagues. Development and initial validation of a scoring system to diagnose testicular torsion in children. J Urol. 2013." },
  acog723: { id: "acog-723", citation: "American College of Obstetricians and Gynecologists. Committee Opinion No. 723. Guidelines for diagnostic imaging during pregnancy and lactation. Obstet Gynecol. 2017." },
  caepCw: { id: "smith-bindman", citation: "Smith-Bindman R and colleagues. Ultrasonography versus computed tomography for suspected nephrolithiasis. N Engl J Med. 2014." },
  eauStone: { id: "eau-stone", citation: "European Association of Urology. EAU guidelines on urolithiasis. 2025 edition." },
  acgPanc: { id: "acg-panc", citation: "Tenner S and colleagues. American College of Gastroenterology guidelines: management of acute pancreatitis. Am J Gastroenterol. 2024." },
  wsesPud: { id: "wses-pud", citation: "Tarasconi A and colleagues. Perforated and bleeding peptic ulcer: WSES guidelines. World J Emerg Surg. 2020." },
  wsesSbo: { id: "wses-sbo", citation: "Ten Broek RPG and colleagues. Bologna guidelines for diagnosis and management of adhesive small bowel obstruction (ASBO): 2017 update of the evidence-based guidelines from the World Society of Emergency Surgery ASBO working group. World J Emerg Surg. 2018." },
};

export const ABDOMINAL_PAIN: Samp[] = [
  {
    id: "abdominal-pain-01",
    topic: "abdominal-pain",
    alsoTopics: ["shock"],
    title: "Older man with sudden flank pain",
    stem:
      "A 74-year-old man arrives at 02:00 with sudden severe left flank pain radiating to the left groin. It began 90 minutes ago. He vomited once. He thinks he is passing a kidney stone, although he has never had one. He has hypertension and smokes one pack per day.\n\nHe is pale and diaphoretic. The abdomen is mildly tender in the left lower quadrant. Urinalysis shows trace blood.",
    vitals: { temperature: "36.4°C", pulse: "112/minute", resp: "22/minute", bp: "96/58 mmHg", o2sat: "96% on room air", weight: "88 kg" },
    questions: [
      {
        id: "q1",
        kind: "short",
        prompt: "What life-threatening diagnoses must you consider before accepting renal colic?",
        required: 3,
        accept: [
          { id: "aaa", text: "Ruptured abdominal aortic aneurysm", match: ["aaa", "aortic aneurysm", "aneurysm", "abdominal aortic"] },
          { id: "mes", text: "Mesenteric ischemia", match: ["mesenteric", "ischemic bowel", "bowel ischemia"] },
          { id: "dis", text: "Aortic dissection", match: ["dissection"] },
          { id: "mi", text: "Acute myocardial infarction", match: ["myocardial infarction", "mi", "acs", "acute coronary", "stemi"] },
          { id: "perf", text: "Perforated viscus", match: ["perforated", "perforation", "free air"] },
          { id: "rp", text: "Retroperitoneal hemorrhage", match: ["retroperitoneal"] },
          { id: "inf", text: "Infected obstructing ureteric stone with urosepsis", match: ["infected", "urosepsis", "pyelonephritis", "obstructed kidney"] },
          { id: "volv", text: "Bowel obstruction or volvulus", match: ["volvulus", "obstruction", "incarcerated hernia"] },
        ],
        explanation:
          "New flank pain in an older smoker with hypotension is a ruptured AAA until proven otherwise. AAA is the classic atypical presentation of flank pain and hematuria mistaken for renal colic. Mesenteric ischemia, dissection and MI must also be kept on the list.",
        keyFeature: { topic: "abdominal-pain", n: 1 },
        source: "svs-aaa",
      },
      {
        id: "q2",
        kind: "single",
        prompt: "Which of the following is the most appropriate next investigation for him?",
        options: [
          "Formal renal ultrasound in the morning",
          "Non-contrast CT of the kidneys, ureters and bladder",
          "Plain abdominal radiograph",
          "Point-of-care ultrasound of the abdominal aorta",
          "Urinalysis with microscopy and culture",
        ],
        correct: 3,
        explanation:
          "Bedside ultrasound identifies an aneurysm within minutes and can be done during resuscitation. A hypotensive patient should not leave the department for CT. Ultrasound does not reliably show rupture, but an aneurysm plus shock is enough to act.",
        keyFeature: { topic: "abdominal-pain", n: 5 },
        source: "tintinalli",
      },
      {
        id: "q3",
        kind: "short",
        update: "Bedside ultrasound shows a 7.2 cm infrarenal aorta. BP is now 84/50 mmHg. He is alert and oriented.",
        prompt: "What are the immediate management steps?",
        required: 3,
        accept: [
          { id: "surg", text: "Call vascular surgery now for the operating room", match: ["vascular surgery", "vascular surgeon", "surgeon", "operating room", "surgery"] },
          { id: "tx", text: "Transfuse blood or activate the massive transfusion protocol", match: ["transfuse", "transfusion", "massive transfusion", "mtp", "prbc", "packed red", "uncrossmatched", "o negative", "o positive", "group o", "blood product"] },
          { id: "perm", text: "Permissive hypotension with a systolic target of 70 to 90 mmHg", match: ["permissive", "hypotensive resuscitation", "target systolic", "sbp 70", "sbp 80", "sbp 90", "limit crystalloid", "limit fluid", "avoid aggressive fluid", "avoid large volume", "avoid overresuscitation", "minimize crystalloid"] },
          { id: "iv", text: "Two large bore IV lines", match: ["large bore", "two iv", "2 iv"] },
          { id: "xm", text: "Crossmatch and send preoperative bloods", match: ["crossmatch", "cross match", "type and cross", "group and screen", "type and screen"] },
          { id: "trans", text: "Arrange immediate transfer to a vascular centre if none on site", match: ["transfer", "vascular centre", "vascular center"] },
          { id: "anes", text: "Notify anesthesia", match: ["anesthesia", "anaesthesia", "anesthetist", "anaesthetist"] },
        ],
        unacceptable: [
          { text: "Send for CT before calling the surgeon", match: ["ct first", "ct before", "cta first", "cta before", "ct scan to confirm"], dangerous: true },
        ],
        explanation:
          "An aneurysm with hypotension needs the operating room, not more imaging. Mobilize surgery, blood and access in parallel. Hypotensive resuscitation avoids diluting clotting factors and raising pressure against a contained rupture. CT is reserved for the stable patient.",
        keyFeature: { topic: "abdominal-pain", n: 4 },
        source: "svs-aaa",
      },
      {
        id: "q4",
        kind: "menu",
        update: "BP is 78/46 mmHg after 500 mL of normal saline. He remains alert. The vascular surgeon is 20 minutes away.",
        prompt: "Which of the following are the most appropriate actions for him now?",
        options: [
          "Give another 2 L of normal saline wide open",
          "Give 1 L of 5% albumin",
          "Give tranexamic acid as the main therapy",
          "Insert a nasogastric tube",
          "Start norepinephrine to a MAP of 80 mmHg",
          "Target a systolic BP above 120 mmHg",
          "Target systolic BP of 70 to 90 mmHg while alert",
          "Transfuse uncrossmatched group O red cells",
        ],
        select: 2,
        correct: [7, 6],
        explanation:
          "Blood is the resuscitation fluid for hemorrhagic shock. Uncrossmatched group O Rh positive red cells are acceptable for a man. Large crystalloid volumes cause dilutional coagulopathy and may worsen bleeding. A systolic target of 70 to 90 mmHg is accepted while mentation is preserved and until the aorta is controlled.",
        keyFeature: { topic: "shock", n: 3 },
        source: "svs-aaa",
      },
    ],
    sources: [SRC.svsAaa, SRC.tintinalli],
    ...meta,
  },
  {
    id: "abdominal-pain-02",
    topic: "abdominal-pain",
    title: "Older woman with severe periumbilical pain",
    stem:
      "A 79-year-old woman presents with severe periumbilical pain that started suddenly 4 hours ago. She vomited twice and had one loose stool. She has atrial fibrillation. She stopped apixaban 2 weeks ago after a nosebleed.\n\nShe is writhing and asking for help. The abdomen is soft with mild diffuse tenderness and no guarding. Lactate is 1.8 mmol/L. WBC is 16.2 x 10^9/L.",
    vitals: { temperature: "36.9°C", pulse: "118/minute irregular", resp: "22/minute", bp: "142/84 mmHg", o2sat: "95% on room air", weight: "61 kg" },
    questions: [
      {
        id: "q1",
        kind: "short",
        prompt: "What serious diagnoses must be excluded?",
        required: 3,
        accept: [
          { id: "mes", text: "Acute mesenteric ischemia", match: ["mesenteric", "ischemic bowel", "bowel ischemia"] },
          { id: "aaa", text: "Abdominal aortic aneurysm", match: ["aaa", "aneurysm"] },
          { id: "panc", text: "Acute pancreatitis", match: ["pancreatitis"] },
          { id: "perf", text: "Perforated viscus", match: ["perforated", "perforation", "free air"] },
          { id: "obs", text: "Bowel obstruction or volvulus", match: ["obstruction", "volvulus", "sbo"] },
          { id: "dis", text: "Aortic dissection", match: ["dissection"] },
          { id: "mi", text: "Myocardial infarction", match: ["myocardial", "mi", "acs", "acute coronary"] },
          { id: "hern", text: "Incarcerated hernia", match: ["hernia"] },
        ],
        unacceptable: [{ text: "Gastroenteritis", match: ["gastroenteritis"] }],
        explanation:
          "Pain out of proportion to a soft abdomen is the classic warning of mesenteric ischemia. Atrial fibrillation off anticoagulation makes embolism likely. Pancreatitis, AAA and perforation can also present this way. Gastroenteritis is a diagnosis of exclusion in an older adult.",
        keyFeature: { topic: "abdominal-pain", n: 2 },
        source: "esvs-mesenteric",
      },
      {
        id: "q2",
        kind: "single",
        prompt: "Which of the following is the most appropriate imaging test for her?",
        options: [
          "CT angiography of the abdomen and pelvis",
          "MRI of the abdomen and pelvis",
          "Non-contrast CT of the abdomen and pelvis",
          "Ultrasound of the abdomen and pelvis",
          "Upright chest and abdominal radiographs",
        ],
        correct: 0,
        explanation:
          "CT angiography is the test of choice for suspected acute mesenteric ischemia. It shows the vessel occlusion and the bowel. Plain films are often normal until infarction. Concern about creatinine should not delay CTA when ischemia is suspected.",
        keyFeature: { topic: "abdominal-pain", n: 5 },
        source: "esvs-mesenteric",
      },
      {
        id: "q3",
        kind: "single",
        update: "The resident suggests ischemia is unlikely because the lactate is normal.",
        prompt: "Which of the following statements about her lactate is most accurate?",
        options: [
          "A lactate under 2 mmol/L rules out acute mesenteric ischemia",
          "A normal lactate does not exclude early mesenteric ischemia",
          "A normal lactate means CT can wait until morning",
          "An elevated lactate is specific for bowel ischemia",
          "Lactate is only useful when the WBC is normal",
        ],
        correct: 1,
        explanation:
          "Lactate is insensitive early in mesenteric ischemia because the liver clears lactate from the portal circulation. It rises late, often after infarction. It is also nonspecific. Decisions rest on clinical suspicion and CTA.",
        keyFeature: { topic: "abdominal-pain", n: 6 },
        source: "esvs-mesenteric",
      },
      {
        id: "q4",
        kind: "short",
        update: "CT angiography shows an embolus occluding the proximal superior mesenteric artery. There is no free air.",
        prompt: "What are the management steps now?",
        required: 3,
        accept: [
          { id: "surg", text: "Emergent vascular and general surgery consultation for revascularization", match: ["vascular", "surgery", "surgeon", "revascularization", "embolectomy", "interventional radiology"] },
          { id: "hep", text: "Unfractionated heparin bolus and infusion", match: ["heparin", "anticoagulation", "anticoagulate"] },
          { id: "abx", text: "Broad spectrum IV antibiotics", match: ["antibiotic", "piperacillin", "pip tazo", "ceftriaxone", "metronidazole", "meropenem"] },
          { id: "fluid", text: "IV fluid resuscitation", match: ["fluid", "crystalloid", "ringer", "saline"] },
          { id: "analg", text: "Opioid analgesia", match: ["analgesia", "opioid", "morphine", "hydromorphone", "fentanyl"] },
          { id: "npo", text: "NPO and nasogastric decompression", match: ["npo", "nothing by mouth", "nasogastric", "ng tube"] },
        ],
        unacceptable: [{ text: "Restart oral apixaban", match: ["apixaban", "doac"] }],
        explanation:
          "Arterial embolic mesenteric ischemia needs urgent revascularization and assessment of bowel viability. Start IV heparin early unless contraindicated. Give antibiotics because of bacterial translocation, and resuscitate. An oral anticoagulant has no role in the acute phase.",
        keyFeature: { topic: "abdominal-pain", n: 2 },
        source: "esvs-mesenteric",
      },
    ],
    sources: [SRC.esvsMes],
    ...meta,
  },
  {
    id: "abdominal-pain-03",
    topic: "abdominal-pain",
    title: "Young woman with right lower quadrant pain",
    stem:
      "A 23-year-old woman has 20 hours of abdominal pain. It began around the umbilicus and has moved to the right lower quadrant. She has anorexia and nausea. Her last menstrual period was 5 weeks ago. She is sexually active and uses condoms inconsistently.\n\nShe has right lower quadrant tenderness with voluntary guarding. There is no costovertebral angle tenderness.",
    vitals: { temperature: "37.2°C", pulse: "96/minute", resp: "16/minute", bp: "118/72 mmHg", o2sat: "99% on room air", weight: "60 kg" },
    questions: [
      {
        id: "q1",
        kind: "short",
        update: "Appendicitis is your leading concern.",
        prompt: "What alternative diagnoses in her would change your first investigations?",
        required: 3,
        accept: [
          { id: "ect", text: "Ectopic pregnancy", match: ["ectopic"] },
          { id: "tor", text: "Ovarian torsion", match: ["torsion"] },
          { id: "cyst", text: "Ruptured or hemorrhagic ovarian cyst", match: ["ovarian cyst", "cyst"] },
          { id: "pid", text: "Pelvic inflammatory disease or tubo-ovarian abscess", match: ["pid", "pelvic inflammatory", "tubo ovarian", "tuboovarian", "toa"] },
          { id: "stone", text: "Ureteric stone", match: ["stone", "renal colic", "ureteric", "nephrolithiasis", "urolithiasis"] },
          { id: "pyelo", text: "Pyelonephritis", match: ["pyelonephritis"] },
          { id: "crohn", text: "Crohn disease or terminal ileitis", match: ["crohn", "ileitis"] },
          { id: "aden", text: "Mesenteric adenitis", match: ["adenitis"] },
          { id: "endo", text: "Endometriosis", match: ["endometriosis", "endometrioma"] },
        ],
        explanation:
          "Location, migration and sex shape the differential. In a woman of reproductive age, ectopic pregnancy, torsion and PID must be considered alongside appendicitis. A pregnancy test comes first.",
        keyFeature: { topic: "abdominal-pain", n: 1 },
        source: "acr-rlq",
      },
      {
        id: "q2",
        kind: "single",
        update: "Serum beta hCG is negative. WBC is 9.4 x 10^9/L. The resident says appendicitis is unlikely because she is afebrile with a normal white count.",
        prompt: "Which of the following statements about her temperature and white count is most accurate?",
        options: [
          "A normal CRP in 6 hours would exclude appendicitis",
          "A normal temperature and WBC do not rule out appendicitis",
          "Appendicitis is excluded when both values are normal",
          "Only fever above 38.5°C is a reliable sign of appendicitis",
          "She can be discharged if the pain improves with analgesia",
        ],
        correct: 1,
        explanation:
          "Fever and leukocytosis are each absent in a large share of confirmed appendicitis, especially early. No single finding is sensitive enough to exclude it. Migration of pain and localized peritonism carry more weight. Analgesia does not mask the diagnosis.",
        keyFeature: { topic: "abdominal-pain", n: 6 },
        source: "wses-appy",
      },
      {
        id: "q3",
        kind: "short",
        prompt: "What is the most appropriate first imaging test for her?",
        required: 1,
        accept: [
          { id: "us", text: "Ultrasound of the right lower quadrant and pelvis", match: ["ultrasound", "us", "sonography", "sonogram", "transvaginal"] },
        ],
        unacceptable: [{ text: "CT abdomen and pelvis as the first test", match: ["ct", "computed tomography"] }],
        explanation:
          "In a young woman, ultrasound avoids ionizing radiation and also assesses the ovaries and adnexa. CT or MRI is used when ultrasound is nondiagnostic and doubt remains. Lifetime cancer risk from CT is highest in the young.",
        keyFeature: { topic: "abdominal-pain", n: 7 },
        source: "acr-rlq",
      },
      {
        id: "q4",
        kind: "short",
        update: "Urinalysis shows 10 to 20 WBC per high power field, no nitrites and no bacteria. She has no dysuria. The resident proposes treating a urinary tract infection.",
        prompt: "Why should this urinalysis not change your working diagnosis?",
        required: 2,
        accept: [
          { id: "adj", text: "An inflamed appendix next to the ureter or bladder causes sterile pyuria", match: ["adjacent", "ureter", "bladder", "next to", "sterile pyuria", "inflamed appendix", "irritation"] },
          { id: "sym", text: "She has no urinary symptoms", match: ["no urinary symptom", "no dysuria", "no frequency", "absence dysuria", "without dysuria", "without urinary", "lack of urinary", "absent urinary", "no symptom"] },
          { id: "nit", text: "No nitrites or bacteria are seen", match: ["nitrite", "no nitrite", "no bacteria", "negative nitrite"] },
          { id: "clin", text: "Migratory pain and localized guarding fit appendicitis", match: ["migration", "migratory", "guarding", "peritoneal", "peritonism", "classic", "clinical picture"] },
          { id: "ns", text: "Pyuria is nonspecific and may reflect contamination", match: ["nonspecific", "non specific", "contamination", "contaminated"] },
        ],
        explanation:
          "Mild pyuria is common in appendicitis because the inflamed appendix irritates the ureter or bladder. Without urinary symptoms, nitrites or bacteria, a UTI does not explain localized peritonism. Labelling abdominal pain as UTI in women is a common cause of missed appendicitis.",
        keyFeature: { topic: "abdominal-pain", n: 8 },
        source: "wses-appy",
      },
    ],
    sources: [SRC.acrRlq, SRC.wsesAppy],
    ...meta,
  },
  {
    id: "abdominal-pain-04",
    topic: "abdominal-pain",
    alsoTopics: ["infectious-diseases"],
    title: "Child with fever and abdominal pain",
    stem:
      "A 7-year-old boy has 1 day of abdominal pain, fever and two episodes of vomiting. He has a mild cough. His parents are worried about his appendix.\n\nThe abdomen is soft with mild diffuse tenderness and no guarding. He can jump without pain. Air entry is reduced at the right base with crackles.",
    vitals: { temperature: "39.4°C", pulse: "136/minute", resp: "34/minute", bp: "102/64 mmHg", o2sat: "92% on room air", weight: "26 kg" },
    questions: [
      {
        id: "q1",
        kind: "short",
        prompt: "What are the causes of abdominal pain in children that originate outside the abdomen?",
        required: 3,
        accept: [
          { id: "pna", text: "Lower lobe pneumonia", match: ["pneumonia"] },
          { id: "strep", text: "Streptococcal pharyngitis", match: ["strep", "pharyngitis", "tonsillitis", "sore throat"] },
          { id: "dka", text: "Diabetic ketoacidosis", match: ["dka", "diabetic ketoacidosis", "ketoacidosis"] },
          { id: "tor", text: "Testicular torsion", match: ["torsion", "testicular"] },
          { id: "hsp", text: "IgA vasculitis (Henoch-Schonlein purpura)", match: ["hsp", "henoch", "iga vasculitis"] },
          { id: "mig", text: "Abdominal migraine", match: ["migraine"] },
          { id: "uri", text: "Otitis media or upper respiratory infection", match: ["otitis", "uri", "upper respiratory"] },
          { id: "tox", text: "Toxic ingestion such as iron or lead", match: ["ingestion", "lead poisoning", "iron", "poisoning", "toxic"] },
          { id: "sickle", text: "Sickle cell crisis", match: ["sickle"] },
        ],
        explanation:
          "Children often localize pain poorly and refer thoracic, pharyngeal and metabolic problems to the abdomen. Lower lobe pneumonia is a classic mimic of appendicitis. Examine the chest, throat and genitals in every child with abdominal pain.",
        keyFeature: { topic: "abdominal-pain", n: 3 },
        source: "tintinalli",
      },
      {
        id: "q2",
        kind: "short",
        prompt: "What is the single most useful investigation now?",
        required: 1,
        accept: [
          { id: "cxr", text: "Chest x-ray", match: ["chest x ray", "cxr", "chest radiograph", "chest xray", "chest film"] },
          { id: "lus", text: "Lung point-of-care ultrasound", match: ["lung ultrasound", "lung pocus"] },
        ],
        unacceptable: [
          { text: "CT abdomen", match: ["ct"] },
          { text: "Abdominal ultrasound for appendicitis", match: ["abdominal ultrasound", "appendix ultrasound", "abdo ultrasound"] },
        ],
        explanation:
          "Tachypnea, hypoxia and focal crackles point to pneumonia. A chest radiograph confirms it and avoids abdominal imaging. An abdominal ultrasound is reasonable only if the abdominal findings evolve.",
        keyFeature: { topic: "abdominal-pain", n: 3 },
        source: "cps-cap",
      },
      {
        id: "q3",
        kind: "short",
        update: "The chest x-ray shows right lower lobe consolidation. After acetaminophen he looks well. SpO2 is 94% on room air and he is drinking.",
        prompt: "Which antibiotic and total daily dose in mg/kg would you prescribe for outpatient treatment?",
        required: 1,
        accept: [
          { id: "amox", text: "Amoxicillin 40 to 90 mg/kg/day orally divided three times daily, maximum 4 g/day", match: ["amoxicillin 90", "amoxicillin 80", "amoxicillin 75", "amoxicillin 60", "amoxicillin 50", "amoxicillin 45", "amoxicillin 40", "amoxicillin 2340", "amoxicillin 2.3 g", "amoxicillin 780", "amoxicillin 1040", "amoxicillin 1170", "amoxicillin 1200"] },
        ],
        unacceptable: [
          { text: "Azithromycin or another macrolide as first line", match: ["azithromycin", "macrolide", "clarithromycin"] },
          { text: "Amoxicillin below the recommended range", match: ["amoxicillin 20", "amoxicillin 25", "amoxicillin 250"] },
        ],
        explanation:
          "The CPS recommends amoxicillin 40 to 90 mg/kg/day divided three times daily, to a maximum of 4 g per day. The higher end covers Streptococcus pneumoniae with reduced penicillin susceptibility. For this 26 kg child, 90 mg/kg/day is 2 340 mg per day, about 780 mg three times daily. Macrolides are not first line for typical bacterial pneumonia in school age children.",
        keyFeature: { topic: "infectious-diseases", n: 8 },
        source: "cps-cap",
      },
      {
        id: "q4",
        kind: "single",
        update: "The parents ask for a CT scan to be sure the appendix is normal.",
        prompt: "Which of the following is the most appropriate response to the parents?",
        options: [
          "Admit him for serial CT scans of the abdomen",
          "Order a low dose CT of the chest and abdomen",
          "Order an abdominal x-ray to exclude appendicitis",
          "Order CT abdomen with contrast to reassure the parents",
          "No CT, with return if the pain localizes or worsens",
        ],
        correct: 4,
        explanation:
          "Pneumonia explains his findings, so CT is not needed. Children are more radiosensitive and have more years to express radiation induced cancer, so CT carries a higher lifetime cancer risk in children. When a clear extra-abdominal cause explains the picture, CT adds risk without benefit. Safety netting, with return if the pain localizes or worsens, and reassessment are the right tools.",
        keyFeature: { topic: "abdominal-pain", n: 7 },
        source: "pearce-ct",
      },
    ],
    sources: [SRC.tintinalli, SRC.cpsCap, SRC.imageGently],
    ...meta,
  },
  {
    id: "abdominal-pain-05",
    topic: "abdominal-pain",
    alsoTopics: ["ischemic-heart-disease"],
    title: "Man with epigastric pain and nausea",
    stem:
      "A 61-year-old man with type 2 diabetes has 90 minutes of epigastric pain and nausea. An antacid at home gave slight relief. He is sweaty.\n\nThe abdomen is soft with mild epigastric tenderness.",
    vitals: { temperature: "36.6°C", pulse: "54/minute", resp: "20/minute", bp: "104/66 mmHg", o2sat: "95% on room air", weight: "92 kg" },
    questions: [
      {
        id: "q1",
        kind: "short",
        prompt: "What investigations should be obtained within 10 minutes of arrival?",
        required: 2,
        accept: [
          { id: "ecg", text: "12 lead ECG", match: ["ecg", "ekg", "electrocardiogram"] },
          { id: "trop", text: "Troponin", match: ["troponin", "trop"] },
          { id: "glu", text: "Capillary glucose", match: ["glucose", "sugar"] },
        ],
        explanation:
          "Epigastric pain in a diabetic man with bradycardia and diaphoresis is ACS until proven otherwise. An ECG within 10 minutes is the priority. Antacid response does not distinguish cardiac from gastric pain.",
        keyFeature: { topic: "abdominal-pain", n: 3 },
        source: "ccs-stemi",
      },
      {
        id: "q2",
        kind: "menu",
        update: "The ECG shows 3 mm ST elevation in II, III and aVF with reciprocal ST depression in aVL. A primary PCI centre is 25 minutes away.",
        prompt: "Which of the following are the most appropriate immediate treatments for him?",
        options: [
          "Activate catheterization lab for primary PCI",
          "ASA 160 mg chewed",
          "ASA 81 mg swallowed",
          "Metoprolol 5 mg IV",
          "Morphine 10 mg IV",
          "Nitroglycerin 0.4 mg SL immediately",
          "Pantoprazole 80 mg IV",
          "Tenecteplase IV bolus now",
          "Viscous lidocaine and antacid",
        ],
        select: 2,
        correct: [1, 0],
        explanation:
          "Chewed ASA 160 to 325 mg and rapid primary PCI are the core of STEMI care when PCI is available within 120 minutes. Fibrinolysis is for when timely PCI is not possible. Beta blockers are harmful with bradycardia and borderline pressure.",
        keyFeature: { topic: "ischemic-heart-disease", n: 6 },
        source: "ccs-stemi",
      },
      {
        id: "q3",
        kind: "short",
        update: "The nurse asks about nitroglycerin for his pain.",
        prompt: "What must you check before nitroglycerin is given?",
        required: 2,
        accept: [
          { id: "rv", text: "Right sided ECG for right ventricular infarction", match: ["right sided", "v4r", "right ventricular", "rv infarct", "rv involvement", "right ecg"] },
          { id: "bp", text: "Blood pressure", match: ["blood pressure", "bp", "systolic", "hypotension"] },
          { id: "pde5", text: "Recent phosphodiesterase 5 inhibitor use", match: ["pde5", "pde 5", "phosphodiesterase", "sildenafil", "tadalafil", "vardenafil", "erectile"] },
          { id: "hr", text: "Heart rate for bradycardia", match: ["heart rate", "bradycardia"] },
        ],
        explanation:
          "Inferior STEMI often involves the right ventricle, which is preload dependent. Nitrates can cause profound hypotension in RV infarction, with low BP, or after a PDE5 inhibitor. His pressure is already borderline.",
        keyFeature: { topic: "ischemic-heart-disease", n: 7 },
        source: "ccs-stemi",
      },
      {
        id: "q4",
        kind: "short",
        prompt: "What other causes outside the abdomen can present as epigastric pain in adults?",
        required: 3,
        accept: [
          { id: "pe", text: "Pulmonary embolism", match: ["pulmonary embolism", "pe"] },
          { id: "pna", text: "Lower lobe pneumonia", match: ["pneumonia"] },
          { id: "dka", text: "Diabetic ketoacidosis", match: ["dka", "ketoacidosis"] },
          { id: "dis", text: "Aortic dissection", match: ["dissection"] },
          { id: "peri", text: "Pericarditis or myocarditis", match: ["pericarditis", "myocarditis"] },
          { id: "zos", text: "Herpes zoster", match: ["zoster", "shingles"] },
          { id: "ptx", text: "Pneumothorax", match: ["pneumothorax"] },
          { id: "eso", text: "Esophageal rupture", match: ["boerhaave", "esophageal rupture", "esophageal perforation", "oesophageal"] },
          { id: "adr", text: "Adrenal crisis", match: ["adrenal", "addison"] },
          { id: "hyperca", text: "Hypercalcemia", match: ["hypercalcemia", "calcium"] },
        ],
        unacceptable: [{ text: "Gastritis or peptic ulcer", match: ["gastritis", "peptic", "gerd", "reflux"] }],
        explanation:
          "Thoracic, metabolic and dermatologic conditions commonly refer pain to the epigastrium. When no clear abdominal cause is found, search the chest and the chemistry. Gastritis and ulcer disease are abdominal causes.",
        keyFeature: { topic: "abdominal-pain", n: 3 },
        source: "rosen",
      },
    ],
    sources: [SRC.ccsStemi, SRC.rosen],
    ...meta,
  },
  {
    id: "abdominal-pain-06",
    topic: "abdominal-pain",
    alsoTopics: ["shock"],
    title: "Young woman with vomiting and abdominal pain",
    stem:
      "A 19-year-old woman has 2 days of vomiting and diffuse abdominal pain. She has had thirst and frequent urination for 3 weeks and has lost 4 kg. She has no medical history.\n\nRespirations are deep.\n\nThe abdomen is diffusely tender without guarding. Venous gas: pH 7.08, bicarbonate 6 mmol/L. Glucose 31 mmol/L. Sodium 131, potassium 5.6, chloride 96 mmol/L. Creatinine 118 umol/L. Beta hydroxybutyrate 7.2 mmol/L. Lipase is normal. Beta hCG is negative.",
    vitals: { temperature: "37.1°C", pulse: "128/minute", resp: "32/minute", bp: "98/60 mmHg", o2sat: "99% on room air", weight: "58 kg" },
    questions: [
      {
        id: "q1",
        kind: "short",
        prompt: "What is the most likely cause of her abdominal pain?",
        required: 1,
        accept: [{ id: "dka", text: "Diabetic ketoacidosis", match: ["dka", "diabetic ketoacidosis", "ketoacidosis"] }],
        unacceptable: [{ text: "Appendicitis", match: ["appendicitis"] }],
        explanation:
          "Abdominal pain is common in DKA and correlates with the severity of acidosis. This is new onset type 1 diabetes with an anion gap of 29. A surgical cause should be sought only if pain persists after the acidosis corrects.",
        keyFeature: { topic: "abdominal-pain", n: 3 },
        source: "dc-dka",
      },
      {
        id: "q2",
        kind: "short",
        prompt: "What are the priorities in her initial management?",
        required: 3,
        accept: [
          { id: "fluid", text: "IV 0.9% saline bolus to restore circulating volume", match: ["normal saline", "0.9%", "saline", "ns", "fluid", "crystalloid", "ringer", "bolus"] },
          { id: "ins", text: "IV insulin infusion", match: ["insulin"] },
          { id: "k", text: "Monitor and replace potassium", match: ["potassium", "k", "kcl"] },
          { id: "mon", text: "Frequent glucose, electrolyte and gas monitoring", match: ["monitor", "monitoring", "hourly", "repeat gas", "repeat vbg", "q1h"] },
          { id: "dex", text: "Add dextrose when glucose falls to about 14 mmol/L", match: ["dextrose", "d5", "d5w"] },
          { id: "prec", text: "Look for a precipitant such as infection", match: ["precipitant", "trigger", "infection"] },
        ],
        unacceptable: [{ text: "Sodium bicarbonate at this pH", match: ["sodium bicarbonate", "give bicarbonate", "bicarbonate infusion", "iv bicarbonate", "bicarb"] }],
        explanation:
          "Volume first, then insulin, with close attention to potassium. Total body potassium is depleted even when the serum level is high. Bicarbonate is generally reserved for pH below about 7.0.",
        keyFeature: { topic: "shock", n: 3 },
        source: "dc-dka",
      },
      {
        id: "q3",
        kind: "single",
        prompt: "Which of the following insulin and potassium plans is most appropriate now?",
        options: [
          "Delay insulin until potassium is below 4.0 mmol/L",
          "Insulin 0.1 units/kg/h, no bolus, KCl held above 5.0 mmol/L",
          "Insulin 0.1 units/kg/h with KCl 40 mmol/L added now",
          "Rapid acting insulin 10 units subcutaneously every two hours",
          "Regular insulin 10 units IV bolus, then 0.1 units/kg/h",
        ],
        correct: 1,
        explanation:
          "A regular insulin infusion of 0.1 units/kg/h is standard and a bolus is not needed. Hold potassium replacement while potassium is above 5.0 mmol/L and recheck it often. Insulin is withheld only for hypokalemia, below 3.3 mmol/L in Diabetes Canada guidance and below 3.5 mmol/L in the 2024 ADA consensus. Both start potassium once it falls below 5.0 mmol/L, which it will with insulin.",
        keyFeature: { topic: "shock", n: 8 },
        source: "dc-dka",
      },
      {
        id: "q4",
        kind: "single",
        update: "Eight hours later the pH is 7.29 and the anion gap has closed. Her abdominal pain has resolved. The abdomen is soft and nontender. The surgical resident asks whether CT of the abdomen is still needed.",
        prompt: "Which of the following is the most appropriate response about abdominal imaging?",
        options: [
          "Abdominal ultrasound before she is discharged",
          "Contrast CT to exclude appendicitis before discharge",
          "CT abdomen and pelvis because of her initial pain",
          "Diagnostic laparoscopy by general surgery",
          "No CT, reassess and image only if pain persists",
        ],
        correct: 4,
        explanation:
          "Imaging should answer a real clinical question. Pain that resolves with correction of DKA supports DKA as the cause and is strong evidence against a surgical cause. Unnecessary CT in a 19-year-old adds lifetime cancer risk and contrast exposure.",
        keyFeature: { topic: "abdominal-pain", n: 7 },
        source: "dc-dka",
      },
    ],
    sources: [SRC.dcDka],
    ...meta,
  },
  {
    id: "abdominal-pain-07",
    topic: "abdominal-pain",
    title: "Confused nursing home resident",
    stem:
      "An 83-year-old woman is sent from her long-term care home with confusion and a presumed urinary tract infection. Staff report poor intake for 2 days, one episode of vomiting, and pain when she is moved. She has dementia, hypertension and polymyalgia rheumatica. Medications include metoprolol and prednisone 10 mg daily.\n\nShe has lower abdominal tenderness, worse on the left, with voluntary guarding. Urine dipstick shows 2+ leukocytes and positive nitrites. WBC 10.1 x 10^9/L. Lactate 2.9 mmol/L.",
    vitals: { temperature: "37.8°C", pulse: "88/minute", resp: "24/minute", bp: "102/60 mmHg", o2sat: "94% on room air" },
    questions: [
      {
        id: "q1",
        kind: "short",
        prompt: "What serious intra-abdominal diagnoses should you consider before attributing her illness to a urinary tract infection?",
        required: 3,
        accept: [
          { id: "div", text: "Complicated diverticulitis", match: ["diverticulitis", "diverticular"] },
          { id: "perf", text: "Perforated viscus", match: ["perforated", "perforation", "free air"] },
          { id: "obs", text: "Bowel obstruction or volvulus", match: ["obstruction", "volvulus", "sbo"] },
          { id: "mes", text: "Mesenteric ischemia", match: ["mesenteric", "ischemic bowel", "bowel ischemia", "ischemic colitis"] },
          { id: "aaa", text: "Abdominal aortic aneurysm", match: ["aaa", "aneurysm"] },
          { id: "bil", text: "Cholecystitis or cholangitis", match: ["cholecystitis", "cholangitis", "biliary"] },
          { id: "app", text: "Appendicitis", match: ["appendicitis"] },
          { id: "hern", text: "Incarcerated hernia", match: ["hernia"] },
          { id: "cdiff", text: "Clostridioides difficile colitis", match: ["difficile", "c diff", "colitis"] },
        ],
        explanation:
          "Localized tenderness with guarding and a raised lactate are not explained by a UTI. Older adults with serious abdominal disease often present with confusion alone. Steroids and a beta blocker can blunt peritonism and tachycardia.",
        keyFeature: { topic: "abdominal-pain", n: 8 },
        source: "rosen",
      },
      {
        id: "q2",
        kind: "short",
        prompt: "Why is the positive urine dipstick unreliable in her?",
        required: 2,
        accept: [
          { id: "asb", text: "Asymptomatic bacteriuria is common in older women in long-term care", match: ["asymptomatic bacteriuria", "asymptomatic", "colonization", "colonized", "colonised", "baseline bacteriuria"] },
          { id: "adj", text: "Adjacent pelvic inflammation can cause pyuria", match: ["adjacent", "sterile pyuria", "pelvic inflammation", "nearby inflammation"] },
          { id: "dem", text: "Dementia prevents her from reporting urinary symptoms", match: ["dementia", "cannot report", "unable to report", "no urinary symptom", "no dysuria"] },
          { id: "exam", text: "Localized tenderness and guarding are not explained by UTI", match: ["guarding", "localized", "localised", "peritoneal", "exam"] },
          { id: "cont", text: "The specimen may be contaminated", match: ["contamination", "contaminated"] },
        ],
        explanation:
          "Bacteriuria and pyuria are present in a large share of women in long-term care without infection. A positive dipstick has poor positive predictive value here. Treating it as the diagnosis delays recognition of surgical disease.",
        keyFeature: { topic: "abdominal-pain", n: 8 },
        source: "idsa-asb",
      },
      {
        id: "q3",
        kind: "short",
        prompt: "What is the most appropriate imaging test?",
        required: 1,
        accept: [{ id: "ct", text: "CT abdomen and pelvis with IV contrast", match: ["ct", "computed tomography"] }],
        unacceptable: [
          { text: "Abdominal radiograph", match: ["x ray", "xray", "radiograph", "plain film"] },
          { text: "Ultrasound", match: ["ultrasound"] },
        ],
        explanation:
          "CT with IV contrast is the best single test for undifferentiated abdominal pain in older adults, especially with left lower quadrant findings. Plain films and ultrasound miss diverticular abscess and small amounts of free air.",
        keyFeature: { topic: "abdominal-pain", n: 5 },
        source: "sis-iai",
      },
      {
        id: "q4",
        kind: "short",
        update: "CT shows perforated sigmoid diverticulitis with free air and pelvic fluid. BP is now 88/50 mmHg.",
        prompt: "What are the immediate management steps?",
        required: 3,
        accept: [
          { id: "surg", text: "Urgent general surgery consultation", match: ["surgery", "surgeon", "surgical"] },
          { id: "abx", text: "Broad spectrum IV antibiotics such as piperacillin-tazobactam", match: ["antibiotic", "piperacillin", "pip tazo", "ceftriaxone", "metronidazole", "meropenem"] },
          { id: "fluid", text: "IV crystalloid bolus", match: ["fluid", "crystalloid", "ringer", "saline", "bolus"] },
          { id: "ster", text: "Stress dose hydrocortisone 100 mg IV", match: ["hydrocortisone", "stress dose", "steroid"] },
          { id: "goc", text: "Goals of care discussion with her substitute decision maker", match: ["goals of care", "advance directive", "code status", "substitute decision", "sdm"] },
          { id: "cult", text: "Blood cultures", match: ["blood culture", "culture"] },
          { id: "analg", text: "Analgesia", match: ["analgesia", "opioid", "morphine", "hydromorphone", "fentanyl"] },
        ],
        unacceptable: [{ text: "Treat as a urinary tract infection only", match: ["nitrofurantoin", "uti only"] }],
        explanation:
          "Perforation with hypotension needs source control, antibiotics and resuscitation at once. Chronic prednisone use calls for stress dose steroids in shock. Her goals of care should guide the surgical plan.",
        keyFeature: { topic: "abdominal-pain", n: 4 },
        source: "sis-iai",
      },
    ],
    sources: [SRC.rosen, SRC.idsaAsb, SRC.sisIai],
    ...meta,
  },
  {
    id: "abdominal-pain-08",
    topic: "abdominal-pain",
    alsoTopics: ["first-trimester-bleeding", "multiple-trauma"],
    title: "Abdominal pain after a bicycle fall",
    stem:
      "A 29-year-old woman, G2P1 at 13 weeks and 1 day, is brought to the emergency department by ambulance 50 minutes after she fell from her bicycle and landed on the handlebar. She has left upper abdominal pain and pain at the tip of her left shoulder. She has had no vaginal bleeding. An ultrasound at 8 weeks showed a live intrauterine pregnancy. She has no medical history and takes a prenatal vitamin. She is pale and anxious. There is an abrasion over the left upper quadrant, with tenderness and guarding. The pelvis is stable. The rest of the primary survey is normal.",
    vitals: { temperature: "36.5°C", pulse: "126/minute", resp: "24/minute", bp: "92/58 mmHg", o2sat: "98% on room air", weight: "64 kg" },
    questions: [
      {
        id: "q1",
        kind: "short",
        required: 3,
        prompt: "What intra-abdominal causes of her pain and hypotension would you consider?",
        accept: [
          { id: "spleen", text: "Splenic injury", match: ["spleen", "splenic"] },
          { id: "liver", text: "Liver injury", match: ["liver", "hepatic"] },
          { id: "ectopic", text: "Heterotopic or ectopic pregnancy", match: ["ectopic", "heterotopic"] },
          { id: "cl", text: "Ruptured corpus luteum or ovarian cyst", match: ["corpus luteum", "ovarian cyst", "cyst rupture", "ruptured cyst"] },
          { id: "bowel", text: "Bowel or mesenteric injury", match: ["bowel", "mesenteric", "mesentery", "hollow viscus", "perforation"] },
          { id: "panc", text: "Pancreatic or duodenal injury", match: ["pancreas", "pancreatic", "duodenal", "duodenum"] },
          { id: "kidney", text: "Kidney injury", match: ["kidney", "renal"] },
          { id: "uterus", text: "Uterine injury or pregnancy loss", match: ["uterine", "uterus", "miscarriage", "abortion", "pregnancy loss"] },
          { id: "diaph", text: "Diaphragm injury", match: ["diaphragm"] },
        ],
        explanation:
          "A handlebar blow to the left upper quadrant with pain at the tip of the left shoulder, tachycardia and hypotension makes a splenic injury the leading cause. The liver, bowel, mesentery, pancreas, duodenum and kidney can also bleed or leak after this mechanism. Pregnancy should widen the list, not narrow it to obstetric causes. A live intrauterine pregnancy at 8 weeks makes an ectopic pregnancy very unlikely, although a heterotopic pregnancy or a ruptured corpus luteum can still bleed. The SOGC trauma guideline warns that the larger blood volume of pregnancy can delay tachycardia and hypotension, so her vital signs point to substantial blood loss.",
        keyFeature: { topic: "abdominal-pain", n: 1 },
        source: "sogc-trauma-325",
      },
      {
        id: "q2",
        kind: "short",
        required: 1,
        prompt: "What imaging would you do first?",
        accept: [{ id: "fast", text: "FAST or extended FAST at the bedside", match: ["fast", "efast", "e fast", "ultrasound", "pocus", "sonography"] }],
        unacceptable: [{ text: "CT before she is stable", match: ["ct abdomen", "ct scan", "computed tomography", "ct of the abdomen"] }],
        explanation:
          "She is hypotensive after blunt abdominal trauma, so the first image must be done at the bedside and answer one question: is there free fluid? The SOGC trauma guideline describes FAST as rapid and free of ionizing radiation, with a sensitivity for free fluid in pregnant patients similar to that in patients who are not pregnant, and recommends it in pregnant patients with major trauma. CT gives organ detail, but a patient in shock should not leave the resuscitation room for it. Fear of radiation is not the reason, because the guideline says maternal imaging must not be delayed for fetal concerns.",
        keyFeature: { topic: "abdominal-pain", n: 5 },
        source: "sogc-trauma-325",
      },
      {
        id: "q3",
        kind: "short",
        required: 3,
        update:
          "FAST shows free fluid in the left upper quadrant and the pelvis. After 1 L of Ringer's lactate her pulse is 132/minute and her BP 84/50 mmHg. Her blood group is not yet known.",
        prompt: "What would you do now?",
        accept: [
          { id: "surg", text: "Call the trauma surgeon for immediate laparotomy", match: ["trauma surgeon", "general surgeon", "surgeon", "surgery", "laparotomy", "operating room", "theatre", "splenectomy"] },
          { id: "mhp", text: "Activate the massive hemorrhage protocol", match: ["massive", "mhp", "mtp", "massive transfusion", "massive hemorrhage"] },
          { id: "oneg", text: "Transfuse O RhD-negative red cells until crossmatched blood is ready", match: ["o negative", "o neg", "uncrossmatched", "emergency release", "red cells", "prbc", "packed red", "transfuse", "blood transfusion"] },
          { id: "noct", text: "Go to the operating room without CT", match: ["no ct", "skip ct", "avoid ct", "without ct", "defer ct", "bypass ct"] },
          { id: "txa", text: "Tranexamic acid 1 g IV", match: ["tranexamic", "txa"] },
          { id: "obs", text: "Notify obstetrics", match: ["obstetric", "obstetrician", "obstetrics", "gynecology", "gynecologist", "obgyn"] },
          { id: "access", text: "Two large-bore IVs with crossmatch", match: ["large bore", "two iv", "crossmatch", "cross match", "group and screen", "type and screen"] },
        ],
        unacceptable: [
          { text: "CT abdomen before surgery", match: ["ct abdomen", "ct scan", "computed tomography", "ct of the abdomen"] },
          { text: "Vasopressor to raise her BP", match: ["vasopressor", "norepinephrine", "phenylephrine"] },
        ],
        explanation:
          "She has free fluid and hypotension that did not respond to a litre of crystalloid, so she needs surgical control of bleeding now. The SOGC trauma guideline states that an exploratory laparotomy should be performed when significant abdominal blood is found, and further imaging would only delay it. Blood, not more crystalloid, is the resuscitation fluid, and the guideline recommends O-negative red cells until crossmatched blood is available, to avoid Rh sensitization. Obstetrics should be told, but her care is driven by her own injury, because the fetus depends on her circulation. The guideline advises against vasopressors unless hypotension persists despite volume replacement, because they can reduce placental perfusion.",
        keyFeature: { topic: "abdominal-pain", n: 4 },
        source: "sogc-trauma-325",
      },
      {
        id: "q4",
        kind: "single",
        update:
          "She undergoes splenectomy and remains pregnant, with a fetal heart rate of 160/minute. Her blood group is O RhD-negative with a negative antibody screen.",
        prompt: "Which of the following is the most appropriate step to prevent Rh D alloimmunization?",
        options: [
          "Kleihauer-Betke test to set the dose",
          "No Rh immune globulin before 20 weeks",
          "Rh immune globulin 120 mcg IM",
          "Rh immune globulin 300 mcg IM",
          "Rh immune globulin only if she bleeds",
        ],
        correct: 3,
        explanation:
          "The SOGC trauma guideline recommends anti-D immune globulin for every RhD-negative pregnant trauma patient, and a single 300 mcg dose within 72 hours covers up to 30 mL of fetal blood. Blunt abdominal trauma can cause fetomaternal hemorrhage without vaginal bleeding, so waiting for bleeding misses the exposure. The 2024 SOGC Rh guideline suggests 300 mcg for sensitizing pregnancy events after 12 weeks, and the 120 mcg minimum applies only in the first 12 weeks. The fetoplacental blood volume is about 120 mL per kg of fetal weight, only a few millilitres at 13 weeks, so a Kleihauer-Betke test would not change the dose. The Rh guideline advises quantitative testing after 20 weeks.",
        keyFeature: { topic: "first-trimester-bleeding", n: 5 },
        source: "sogc-rh-448",
      },
    ],
    sources: [SRC.sogcTrauma325, SRC.sogcRh448],
    ...meta,
    reviewed: false,
    version: 2,
  },
  {
    id: "abdominal-pain-09",
    topic: "abdominal-pain",
    title: "Teenager with lower abdominal pain and vomiting",
    stem:
      "A 14-year-old boy has 3 hours of severe lower abdominal pain and vomiting. The pain woke him from sleep. He describes it as belly pain and does not mention anything else.\n\nThe abdomen is soft with mild left lower quadrant tenderness.",
    vitals: { temperature: "37.0°C", pulse: "108/minute", resp: "18/minute", bp: "128/76 mmHg", o2sat: "99% on room air", weight: "55 kg" },
    questions: [
      {
        id: "q1",
        kind: "short",
        update: "You examine the genitals.",
        prompt: "What findings would support testicular torsion?",
        required: 3,
        accept: [
          { id: "high", text: "High riding testis", match: ["high riding", "elevated", "retracted", "high testis"] },
          { id: "horiz", text: "Horizontal lie", match: ["horizontal", "transverse lie"] },
          { id: "crem", text: "Absent cremasteric reflex", match: ["cremasteric"] },
          { id: "tend", text: "Firm, swollen, tender testis", match: ["swollen", "swelling", "tender testis", "tender testicle", "firm", "hard"] },
          { id: "ery", text: "Scrotal erythema or edema", match: ["erythema", "red", "scrotal edema", "scrotal oedema"] },
        ],
        explanation:
          "Adolescent boys often report torsion as abdominal pain. Every male with lower abdominal pain needs a genital exam. Absent cremasteric reflex, high riding testis and horizontal lie are the key findings.",
        keyFeature: { topic: "abdominal-pain", n: 1 },
        source: "twist",
      },
      {
        id: "q2",
        kind: "single",
        update: "The left testis is high riding, firm and tender, with a horizontal lie and absent cremasteric reflex. Ultrasound is available in 2 hours.",
        prompt: "Which of the following is the most appropriate next step for him?",
        options: [
          "Analgesia and reassessment in 2 hours",
          "CT of the abdomen and pelvis with contrast",
          "Doppler ultrasound first to confirm the diagnosis of torsion",
          "Urgent urology exploration without waiting for ultrasound",
          "Urinalysis and treatment for epididymitis",
        ],
        correct: 3,
        explanation:
          "With a high clinical probability of torsion, imaging only delays surgery. Consult urology urgently for surgical exploration. Testicular salvage falls steeply after about 6 hours. Ultrasound is useful when the diagnosis is genuinely uncertain.",
        keyFeature: { topic: "abdominal-pain", n: 4 },
        source: "twist",
      },
      {
        id: "q3",
        kind: "single",
        update: "His mother says he was seen at a walk-in clinic yesterday for similar pain that settled. A Doppler ultrasound there was reported as normal.",
        prompt: "Which of the following statements about the earlier ultrasound is most accurate?",
        options: [
          "A normal Doppler yesterday does not exclude torsion today",
          "Doppler ultrasound is 100% sensitive for torsion",
          "Intermittent torsion needs no surgical follow up",
          "Normal flow on yesterday's scan makes torsion very unlikely",
          "Torsion cannot present with abdominal pain alone",
        ],
        correct: 0,
        explanation:
          "Intermittent torsion and detorsion can produce normal flow between episodes. Doppler also misses partial torsion. A previous negative test must not override current clinical findings.",
        keyFeature: { topic: "abdominal-pain", n: 6 },
        source: "rosen",
      },
    ],
    sources: [SRC.twist, SRC.rosen],
    ...meta,
  },
  {
    id: "abdominal-pain-10",
    topic: "abdominal-pain",
    alsoTopics: ["shock"],
    title: "Infant with episodes of crying",
    stem:
      "A 10-month-old boy has 12 hours of episodes of inconsolable crying every 15 to 20 minutes. He draws his legs up during each episode. He has vomited four times, not bilious. Between episodes he is pale and sleepy. He passed one stool with dark red mucus.\n\nCapillary refill is 3 seconds.\n\nA vague mass is felt in the right upper quadrant. The abdomen is not peritonitic.",
    vitals: { temperature: "37.6°C", pulse: "168/minute", resp: "36/minute", bp: "84/50 mmHg", o2sat: "98% on room air", weight: "9 kg" },
    questions: [
      {
        id: "q1",
        kind: "short",
        prompt: "What diagnoses should you consider at this age?",
        required: 3,
        accept: [
          { id: "intus", text: "Intussusception", match: ["intussusception"] },
          { id: "malr", text: "Malrotation with midgut volvulus", match: ["malrotation", "volvulus"] },
          { id: "hern", text: "Incarcerated inguinal hernia", match: ["hernia"] },
          { id: "tor", text: "Testicular torsion", match: ["torsion"] },
          { id: "ent", text: "Bacterial enterocolitis", match: ["enteritis", "enterocolitis", "gastroenteritis", "colitis"] },
          { id: "meck", text: "Meckel diverticulum", match: ["meckel"] },
          { id: "hus", text: "Hemolytic uremic syndrome", match: ["hemolytic uremic", "hus"] },
          { id: "nat", text: "Non-accidental abdominal trauma", match: ["abuse", "non accidental", "nonaccidental", "nat", "inflicted"] },
        ],
        explanation:
          "Age drives the differential. Intermittent colicky pain, lethargy between episodes and bloody mucus at 10 months point to intussusception. Volvulus, incarcerated hernia and torsion are time critical and must not be missed.",
        keyFeature: { topic: "abdominal-pain", n: 1 },
        source: "tintinalli",
      },
      {
        id: "q2",
        kind: "short",
        prompt: "What is the most appropriate imaging test?",
        required: 1,
        accept: [{ id: "us", text: "Abdominal ultrasound", match: ["ultrasound", "us", "pocus", "sonography"] }],
        unacceptable: [
          { text: "CT abdomen", match: ["ct", "computed tomography"] },
          { text: "Abdominal x-ray alone", match: ["x ray", "xray", "radiograph"] },
        ],
        explanation:
          "Ultrasound is highly sensitive and specific for intussusception and uses no radiation. CT adds radiation in a highly radiosensitive infant. Plain films are insensitive.",
        keyFeature: { topic: "abdominal-pain", n: 7 },
        source: "pearce-ct",
      },
      {
        id: "q3",
        kind: "short",
        update: "Ultrasound shows a target sign in the right upper quadrant. There is no free fluid.",
        prompt: "What are the next management steps?",
        required: 2,
        accept: [
          { id: "enema", text: "Air or hydrostatic enema reduction by radiology", match: ["enema", "air reduction", "pneumatic", "hydrostatic", "reduction"] },
          { id: "surg", text: "Notify pediatric surgery", match: ["surgery", "surgeon", "surgical"] },
          { id: "fluid", text: "IV fluid resuscitation", match: ["bolus", "fluid", "saline"] },
          { id: "analg", text: "Analgesia", match: ["analgesia", "morphine", "fentanyl"] },
          { id: "npo", text: "NPO with nasogastric decompression", match: ["npo", "nasogastric", "ng tube", "nothing by mouth"] },
        ],
        explanation:
          "Enema reduction is first line for a stable child without peritonitis or perforation. Surgery must be aware in case reduction fails or perforation occurs. Resuscitate first because these children are often volume depleted.",
        keyFeature: { topic: "abdominal-pain", n: 4 },
        source: "tintinalli",
      },
      {
        id: "q4",
        kind: "single",
        prompt: "Which of the following initial fluid boluses is most appropriate for him?",
        options: [
          "0.45% saline maintenance infusion at 36 mL/h only",
          "0.9% saline 90 mL IV over 1 hour",
          "0.9% saline 180 mL IV over 5 to 20 minutes",
          "0.9% saline 900 mL IV bolus",
          "5% dextrose in water 180 mL IV bolus",
        ],
        correct: 2,
        explanation:
          "Isotonic crystalloid at 20 mL/kg is the standard bolus for a child with poor perfusion, so 180 mL for 9 kg. Reassess after each bolus. Hypotonic or dextrose only fluids are not resuscitation fluids.",
        keyFeature: { topic: "shock", n: 3 },
        source: "tintinalli",
      },
    ],
    sources: [SRC.tintinalli, SRC.imageGently],
    ...meta,
  },
  {
    id: "abdominal-pain-11",
    topic: "abdominal-pain",
    title: "Pregnant woman with right lower quadrant pain",
    stem:
      "A 30-year-old woman, G1P0 at 14 weeks by dating ultrasound, has 24 hours of right lower quadrant pain, nausea and anorexia.\n\nShe has right lower quadrant tenderness with guarding. Fetal heart rate is 156/minute. WBC is 15.0 x 10^9/L.",
    vitals: { temperature: "37.9°C", pulse: "104/minute", resp: "18/minute", bp: "112/68 mmHg", o2sat: "99% on room air" },
    questions: [
      {
        id: "q1",
        kind: "single",
        prompt: "Which of the following is the most appropriate first imaging test for her?",
        options: [
          "CT of the abdomen and pelvis with IV contrast",
          "Graded compression ultrasound of the right lower quadrant",
          "MRI of the abdomen and pelvis with gadolinium",
          "Nuclear medicine scan with indium labelled white blood cells",
          "Supine and upright abdominal radiographs",
        ],
        correct: 1,
        explanation:
          "Ultrasound is first line in pregnancy because it uses no ionizing radiation. A visualized inflamed appendix confirms the diagnosis. A nondiagnostic scan leads to MRI.",
        keyFeature: { topic: "abdominal-pain", n: 7 },
        source: "acog-723",
      },
      {
        id: "q2",
        kind: "short",
        update: "The ultrasound does not visualize the appendix. There is no free fluid.",
        prompt: "What is the most appropriate next imaging test?",
        required: 1,
        accept: [{ id: "mri", text: "Non-contrast MRI of the abdomen and pelvis", match: ["mri", "magnetic resonance"] }],
        unacceptable: [{ text: "MRI with gadolinium", match: ["gadolinium"] }],
        explanation:
          "Non-contrast MRI is highly accurate for appendicitis in pregnancy and avoids radiation. Gadolinium crosses the placenta and should be avoided unless essential.",
        keyFeature: { topic: "abdominal-pain", n: 5 },
        source: "acog-723",
      },
      {
        id: "q3",
        kind: "short",
        update: "MRI is not available at your hospital for 24 hours.",
        prompt: "What are acceptable next steps?",
        required: 2,
        accept: [
          { id: "surg", text: "General surgery consultation for clinical assessment", match: ["surgery", "surgeon", "surgical"] },
          { id: "ct", text: "CT after counselling that the fetal dose is low and below harmful thresholds", match: ["ct", "computed tomography"] },
          { id: "trans", text: "Transfer to a centre with MRI", match: ["transfer"] },
          { id: "obs", text: "Admit for serial examinations and repeat ultrasound", match: ["serial", "repeat ultrasound", "admit", "observe", "observation"] },
          { id: "ob", text: "Obstetrics consultation", match: ["obstetric", "obstetrician", "obstetrics"] },
        ],
        unacceptable: [
          { text: "Discharge home for outpatient MRI tomorrow", match: ["discharge", "outpatient mri", "mri tomorrow"], dangerous: true },
        ],
        explanation:
          "Perforated appendicitis carries a much higher risk of fetal loss than a single CT. The fetal dose from abdominal CT is well below the level linked to fetal harm. Imaging should not be withheld when it is needed to make the diagnosis.",
        keyFeature: { topic: "abdominal-pain", n: 7 },
        source: "acog-723",
      },
      {
        id: "q4",
        kind: "short",
        prompt: "What other causes of right lower quadrant pain should you consider in pregnancy?",
        required: 3,
        accept: [
          { id: "tor", text: "Ovarian torsion", match: ["torsion"] },
          { id: "cyst", text: "Ovarian cyst rupture or hemorrhage", match: ["cyst"] },
          { id: "pyelo", text: "Pyelonephritis", match: ["pyelonephritis"] },
          { id: "stone", text: "Ureteric stone", match: ["stone", "renal colic", "ureteric", "nephrolithiasis"] },
          { id: "rl", text: "Round ligament pain", match: ["round ligament"] },
          { id: "fib", text: "Degenerating fibroid", match: ["fibroid", "leiomyoma"] },
          { id: "bil", text: "Cholecystitis with an upwardly displaced gallbladder", match: ["cholecystitis", "biliary", "gallbladder"] },
          { id: "het", text: "Heterotopic pregnancy", match: ["heterotopic"] },
          { id: "misc", text: "Threatened miscarriage", match: ["miscarriage", "abortion"] },
          { id: "crohn", text: "Crohn disease", match: ["crohn", "ileitis"] },
        ],
        explanation:
          "Pregnancy adds obstetric causes and makes torsion and pyelonephritis more common. The enlarging uterus shifts the appendix and gallbladder, so location becomes less reliable.",
        keyFeature: { topic: "abdominal-pain", n: 1 },
        source: "rosen",
      },
    ],
    sources: [SRC.acog723, SRC.rosen],
    ...meta,
  },
  {
    id: "abdominal-pain-12",
    topic: "abdominal-pain",
    alsoTopics: ["analgesia-sedation", "infectious-diseases"],
    title: "Man with recurrent flank pain",
    stem:
      "A 36-year-old man has 3 hours of colicky left flank pain radiating to the groin. This is his fourth episode of kidney stones. He has had three CT scans for stones in the past 5 years.\n\nThe abdomen is soft and nontender. Urinalysis shows blood and no leukocytes or nitrites. Creatinine is 82 umol/L.",
    vitals: { temperature: "36.8°C", pulse: "96/minute", resp: "18/minute", bp: "138/84 mmHg", o2sat: "99% on room air", weight: "80 kg" },
    questions: [
      {
        id: "q1",
        kind: "single",
        prompt: "Which of the following imaging strategies is most appropriate for him?",
        options: [
          "CT of the abdomen and pelvis with IV contrast",
          "Intravenous pyelogram with delayed images",
          "MRI urogram of the kidneys and ureters",
          "Non-contrast CT of the kidneys, ureters and bladder now",
          "Renal POCUS, with no CT if the course is uncomplicated",
        ],
        correct: 4,
        explanation:
          "A young patient with known stones and a typical, uncomplicated episode does not need repeat CT. Point-of-care renal ultrasound looks for hydronephrosis. Ultrasound first reduces cumulative radiation without worse outcomes. CT is kept for atypical features or complications.",
        keyFeature: { topic: "abdominal-pain", n: 7 },
        source: "smith-bindman",
      },
      {
        id: "q2",
        kind: "short",
        prompt: "What features in a patient with suspected renal colic should prompt urgent imaging or urology involvement?",
        required: 3,
        accept: [
          { id: "inf", text: "Fever or signs of infection", match: ["fever", "infection", "sepsis", "septic", "pyuria"] },
          { id: "sol", text: "Solitary or transplanted kidney", match: ["solitary", "single kidney", "transplant"] },
          { id: "aki", text: "Acute kidney injury", match: ["creatinine", "renal failure", "kidney injury", "aki", "renal function"] },
          { id: "pain", text: "Intractable pain or vomiting", match: ["intractable", "refractory", "uncontrolled", "persistent vomiting", "cannot tolerate"] },
          { id: "age", text: "Older age or first episode, raising concern for AAA", match: ["age", "older", "first episode", "aaa", "aneurysm"] },
          { id: "preg", text: "Pregnancy", match: ["pregnancy", "pregnant"] },
          { id: "unst", text: "Hemodynamic instability", match: ["hypotension", "unstable", "instability", "shock"] },
          { id: "large", text: "Large stone over 10 mm", match: ["large stone", "10 mm", "stone size"] },
        ],
        explanation:
          "An infected obstructed kidney is a urologic emergency. Any threat to renal function also needs urgent imaging. In older patients, a first episode of flank pain must prompt consideration of AAA.",
        keyFeature: { topic: "abdominal-pain", n: 1 },
        source: "eau-stone",
      },
      {
        id: "q3",
        kind: "menu",
        prompt: "Which of the following are most appropriate for his pain and stone passage?",
        options: [
          "Hyoscine butylbromide 20 mg IV",
          "IV normal saline 2 L bolus to flush the stone",
          "Ketorolac 15 mg IV",
          "Ketorolac 60 mg IV",
          "Morphine 20 mg IV push",
          "Oxycodone 5 mg PO only",
          "Tamsulosin 4 mg PO daily",
          "Tamsulosin 0.4 mg PO daily, distal 5 to 10 mm",
        ],
        select: 2,
        correct: [2, 7],
        explanation:
          "NSAIDs are first line for renal colic, and ketorolac shows a ceiling effect at 10 to 15 mg IV. Tamsulosin 0.4 mg daily aids passage of distal stones of 5 to 10 mm, so it is given only if the stone is distal and 5 to 10 mm. Forced IV hydration does not help and antispasmodics add little.",
        keyFeature: { topic: "analgesia-sedation", n: 2 },
        source: "eau-stone",
      },
      {
        id: "q4",
        kind: "short",
        update: "He returns 36 hours later with rigors. Temperature 39.2°C, HR 122/minute, BP 94/58 mmHg. Bedside ultrasound shows moderate left hydronephrosis.",
        prompt: "What are the immediate management steps?",
        required: 3,
        accept: [
          { id: "cult", text: "Blood and urine cultures", match: ["culture"] },
          { id: "abx", text: "IV antibiotics such as ceftriaxone", match: ["antibiotic", "ceftriaxone", "piperacillin", "gentamicin", "ampicillin"] },
          { id: "uro", text: "Urgent urology for decompression by stent or nephrostomy", match: ["urology", "urologist", "stent", "nephrostomy", "decompression", "decompress"] },
          { id: "fluid", text: "IV fluid resuscitation", match: ["fluid", "bolus", "crystalloid", "saline", "ringer"] },
          { id: "lac", text: "Lactate and sepsis bloodwork", match: ["lactate"] },
        ],
        unacceptable: [{ text: "Discharge with oral antibiotics", match: ["oral antibiotic", "discharge"], dangerous: true }],
        explanation:
          "An obstructed infected kidney needs antibiotics and urgent drainage. Antibiotics alone fail because the collecting system cannot drain. Discharging this patient risks rapid progression to septic shock.",
        keyFeature: { topic: "infectious-diseases", n: 1 },
        source: "eau-stone",
      },
    ],
    sources: [SRC.caepCw, SRC.eauStone],
    ...meta,
  },
  {
    id: "abdominal-pain-13",
    topic: "abdominal-pain",
    title: "Man with epigastric pain radiating to the back",
    stem:
      "A 47-year-old man who drinks 12 beers a day has 10 hours of severe epigastric pain radiating to the back, with repeated vomiting.\n\nHe is writhing in pain. There is epigastric tenderness with mild guarding. The rest of the abdomen is soft.",
    vitals: { temperature: "37.8°C", pulse: "118/minute", resp: "22/minute", bp: "108/70 mmHg", o2sat: "95% on room air", weight: "78 kg" },
    questions: [
      {
        id: "q1",
        kind: "short",
        prompt: "What serious diagnoses should be excluded?",
        required: 3,
        accept: [
          { id: "perf", text: "Perforated peptic ulcer", match: ["perforated", "perforation", "free air"] },
          { id: "panc", text: "Acute pancreatitis", match: ["pancreatitis"] },
          { id: "aaa", text: "Abdominal aortic aneurysm", match: ["aaa", "aneurysm"] },
          { id: "mi", text: "Myocardial infarction", match: ["myocardial", "mi", "acs", "acute coronary"] },
          { id: "mes", text: "Mesenteric ischemia", match: ["mesenteric", "bowel ischemia", "ischemic bowel"] },
          { id: "dis", text: "Aortic dissection", match: ["dissection"] },
          { id: "eso", text: "Esophageal rupture", match: ["boerhaave", "esophageal", "oesophageal"] },
          { id: "bil", text: "Cholangitis or cholecystitis", match: ["cholangitis", "cholecystitis", "biliary"] },
        ],
        explanation:
          "Severe pain with few abdominal signs demands exclusion of pancreatitis, perforation and vascular catastrophes. An ECG, lipase and upright chest radiograph are quick first steps. Alcohol use raises the risk of several of these.",
        keyFeature: { topic: "abdominal-pain", n: 2 },
        source: "acg-panc",
      },
      {
        id: "q2",
        kind: "single",
        update: "Lipase is 1 850 U/L. The upper limit of normal is 60 U/L. The ECG shows sinus tachycardia only. An upright chest radiograph shows no free air.",
        prompt: "Which of the following statements about CT to confirm the diagnosis in the ED is most accurate?",
        options: [
          "Contrast CT, required to confirm every case of pancreatitis",
          "CT now, to predict necrosis in the first 12 hours",
          "MRCP before admission to the ward",
          "No imaging at any stage of his illness",
          "No CT, reserved for diagnostic doubt or failure to improve",
        ],
        correct: 4,
        explanation:
          "Two of three criteria make the diagnosis. Typical pain plus lipase above three times normal confirms pancreatitis, so CT is reserved for diagnostic doubt or failure to improve. Early CT underestimates necrosis and rarely changes management. It adds radiation and contrast in a volume depleted patient.",
        keyFeature: { topic: "abdominal-pain", n: 7 },
        source: "acg-panc",
      },
      {
        id: "q3",
        kind: "short",
        prompt: "What investigations would look for a cause other than alcohol?",
        required: 2,
        accept: [
          { id: "us", text: "Right upper quadrant ultrasound for gallstones", match: ["ultrasound", "us", "gallstone"] },
          { id: "tg", text: "Triglycerides", match: ["triglyceride", "lipid"] },
          { id: "ca", text: "Calcium", match: ["calcium"] },
          { id: "lft", text: "Liver enzymes and bilirubin", match: ["alt", "ast", "liver enzyme", "lft", "liver", "bilirubin"] },
        ],
        explanation:
          "Gallstones are the most common cause of pancreatitis and change management, so every patient needs an ultrasound. Hypertriglyceridemia and hypercalcemia are treatable causes. Ultrasound is the right imaging for suspected biliary disease.",
        keyFeature: { topic: "abdominal-pain", n: 5 },
        source: "acg-panc",
      },
      {
        id: "q4",
        kind: "short",
        prompt: "What are the elements of his initial ED management?",
        required: 3,
        accept: [
          { id: "fluid", text: "Moderate goal directed IV fluid with Ringer's lactate", match: ["ringer", "fluid", "crystalloid", "saline"] },
          { id: "analg", text: "Opioid analgesia", match: ["opioid", "analgesia", "hydromorphone", "morphine", "fentanyl"] },
          { id: "ae", text: "Antiemetic", match: ["antiemetic", "ondansetron", "metoclopramide"] },
          { id: "thi", text: "Thiamine", match: ["thiamine"] },
          { id: "wd", text: "Monitor and treat alcohol withdrawal", match: ["withdrawal", "ciwa", "benzodiazepine", "lorazepam", "diazepam"] },
          { id: "feed", text: "Early oral feeding as tolerated", match: ["feeding", "oral intake", "diet", "eat"] },
          { id: "noabx", text: "Avoid prophylactic antibiotics", match: ["avoid antibiotic", "no antibiotic", "no prophylactic antibiotic", "avoid prophylactic antibiotic", "withhold antibiotic"] },
        ],
        unacceptable: [{ text: "Prophylactic antibiotics", match: ["antibiotic", "meropenem", "imipenem"] }],
        explanation:
          "Early care is supportive. Moderate fluid is preferred over aggressive fluid, which causes overload without benefit. Prophylactic antibiotics do not help sterile pancreatitis. Alcohol use calls for thiamine and withdrawal monitoring.",
        keyFeature: { topic: "abdominal-pain", n: 2 },
        source: "acg-panc",
      },
    ],
    sources: [SRC.acgPanc],
    ...meta,
  },
  {
    id: "abdominal-pain-14",
    topic: "abdominal-pain",
    alsoTopics: ["analgesia-sedation"],
    title: "Man with sudden severe epigastric pain",
    stem:
      "A 68-year-old man had sudden severe epigastric pain 3 hours ago. It is now diffuse. He takes naproxen daily for knee osteoarthritis and smokes.\n\nHe lies still. The abdomen is rigid with rebound tenderness.",
    vitals: { temperature: "37.6°C", pulse: "116/minute", resp: "24/minute", bp: "104/66 mmHg", o2sat: "96% on room air", weight: "84 kg" },
    questions: [
      {
        id: "q1",
        kind: "short",
        prompt: "What is the fastest imaging test to support your leading diagnosis?",
        required: 1,
        accept: [
          { id: "cxr", text: "Upright chest x-ray for free air", match: ["upright chest", "erect chest", "chest x ray", "chest xray", "cxr", "chest radiograph"] },
          { id: "pocus", text: "Point-of-care ultrasound for free air", match: ["pocus", "ultrasound"] },
        ],
        explanation:
          "Sudden pain, NSAID use and a rigid abdomen point to a perforated ulcer. An upright chest radiograph shows free air in most cases and can be done at the bedside. A negative film does not exclude perforation.",
        keyFeature: { topic: "abdominal-pain", n: 5 },
        source: "wses-pud",
      },
      {
        id: "q2",
        kind: "short",
        update: "The upright chest x-ray shows free air under the right hemidiaphragm.",
        prompt: "What are the immediate management steps?",
        required: 3,
        accept: [
          { id: "surg", text: "General surgery consultation now", match: ["surgery", "surgeon", "surgical", "operating room"] },
          { id: "abx", text: "Broad spectrum IV antibiotics", match: ["antibiotic", "piperacillin", "ceftriaxone", "metronidazole", "pip tazo"] },
          { id: "ppi", text: "IV proton pump inhibitor such as pantoprazole", match: ["ppi", "pantoprazole", "proton pump", "esomeprazole"] },
          { id: "fluid", text: "IV fluid resuscitation", match: ["fluid", "crystalloid", "ringer", "saline", "bolus"] },
          { id: "ng", text: "NPO and nasogastric tube", match: ["npo", "nothing by mouth", "nasogastric", "ng tube", "ngt"] },
          { id: "analg", text: "Opioid analgesia", match: ["analgesia", "opioid", "morphine", "hydromorphone", "fentanyl"] },
        ],
        explanation:
          "Free air with peritonitis is a surgical emergency. Resuscitation, antibiotics and acid suppression run alongside the call to surgery. Delay to source control increases mortality.",
        keyFeature: { topic: "abdominal-pain", n: 4 },
        source: "wses-pud",
      },
      {
        id: "q3",
        kind: "single",
        update: "The surgical resident asks for a CT abdomen before coming to see him.",
        prompt: "Which of the following is the most appropriate response to the surgical resident?",
        options: [
          "Agree and order CT before surgical assessment",
          "Arrange urgent upper endoscopy by the gastroenterology team",
          "Ask the surgeon to assess him now, without further imaging",
          "Order an abdominal ultrasound first",
          "Repeat the upright chest x-ray in 6 hours",
        ],
        correct: 2,
        explanation:
          "The diagnosis and the need for surgery are already clear. Free air with peritonitis needs operative management without further imaging. CT may help the surgeon plan, but it must not delay surgical assessment. Endoscopy is contraindicated with a perforation.",
        keyFeature: { topic: "abdominal-pain", n: 4 },
        source: "wses-pud",
      },
      {
        id: "q4",
        kind: "single",
        prompt: "Which of the following analgesics is most appropriate for him now?",
        options: [
          "Acetaminophen 1 g PO as the only agent",
          "Hydromorphone 0.5 mg IV, titrated to effect",
          "Ketorolac 30 mg IV every 6 hours",
          "Naproxen 500 mg PO twice daily",
          "Withhold analgesia until the surgeon examines him",
        ],
        correct: 1,
        explanation:
          "Titrated IV opioid is safe and does not mask surgical findings. NSAIDs worsen ulcer disease and bleeding risk. Oral drugs are inappropriate in a patient who needs surgery.",
        keyFeature: { topic: "analgesia-sedation", n: 2 },
        source: "tintinalli",
      },
    ],
    sources: [SRC.wsesPud, SRC.tintinalli],
    ...meta,
  },
  {
    id: "abdominal-pain-15",
    topic: "abdominal-pain",
    title: "Older man with distension and vomiting",
    stem:
      "An 84-year-old man has 2 days of crampy abdominal pain, distension and vomiting. He has not passed flatus for 24 hours. He had an open colectomy 10 years ago. He takes metoprolol.\n\nThe abdomen is distended and tympanic with mild diffuse tenderness and no rebound. WBC is 9.8 x 10^9/L.",
    vitals: { temperature: "37.1°C", pulse: "84/minute", resp: "20/minute", bp: "128/70 mmHg", o2sat: "95% on room air" },
    questions: [
      {
        id: "q1",
        kind: "short",
        update: "A student notes the normal temperature, heart rate and WBC and suggests constipation.",
        prompt: "Why may older adults with serious abdominal disease lack these findings?",
        required: 3,
        accept: [
          { id: "fev", text: "Blunted fever response", match: ["fever", "temperature", "thermoregulation", "afebrile"] },
          { id: "bb", text: "Beta blockers blunt tachycardia", match: ["beta blocker", "metoprolol", "heart rate", "tachycardia"] },
          { id: "wbc", text: "Blunted leukocyte response", match: ["wbc", "white", "leukocytosis", "leukocyte"] },
          { id: "perit", text: "Fewer peritoneal signs because of weaker abdominal wall response", match: ["peritoneal", "guarding", "rigidity", "abdominal wall", "peritonism"] },
          { id: "cog", text: "Cognitive impairment limits the history", match: ["cognitive", "dementia", "delirium", "historian", "communicate"] },
          { id: "drug", text: "Steroids or analgesics mask inflammation", match: ["steroid", "nsaid", "analgesic", "prednisone"] },
        ],
        explanation:
          "Older adults often lack fever, tachycardia, leukocytosis and peritonism even with surgical disease. Normal values in them are weak evidence. Their mortality from missed abdominal disease is high.",
        keyFeature: { topic: "abdominal-pain", n: 6 },
        source: "rosen",
      },
      {
        id: "q2",
        kind: "short",
        prompt: "What is the most appropriate imaging test?",
        required: 1,
        accept: [{ id: "ct", text: "CT abdomen and pelvis with IV contrast", match: ["ct", "computed tomography"] }],
        explanation:
          "CT with IV contrast confirms obstruction, finds the transition point and shows signs of strangulation. Oral contrast is not required. Plain films cannot exclude ischemia.",
        keyFeature: { topic: "abdominal-pain", n: 5 },
        source: "wses-sbo",
      },
      {
        id: "q3",
        kind: "short",
        update: "CT shows a closed loop small bowel obstruction with reduced bowel wall enhancement and mesenteric fluid. Lactate is now 3.8 mmol/L.",
        prompt: "What are the immediate management steps?",
        required: 3,
        accept: [
          { id: "surg", text: "Urgent general surgery for the operating room", match: ["surgery", "surgeon", "surgical", "operating room"] },
          { id: "ng", text: "Nasogastric decompression", match: ["nasogastric", "ng tube", "ngt", "decompression"] },
          { id: "fluid", text: "IV fluid resuscitation", match: ["fluid", "crystalloid", "ringer", "saline"] },
          { id: "abx", text: "IV antibiotics", match: ["antibiotic", "piperacillin", "ceftriaxone", "metronidazole"] },
          { id: "analg", text: "Analgesia and antiemetic", match: ["analgesia", "opioid", "hydromorphone", "morphine", "antiemetic", "ondansetron"] },
          { id: "goc", text: "Goals of care discussion", match: ["goals of care", "code status", "advance directive"] },
          { id: "foley", text: "Urinary catheter to monitor output", match: ["foley", "urine output", "urinary catheter"] },
        ],
        unacceptable: [
          { text: "Water soluble contrast challenge and observation", match: ["gastrografin", "water soluble", "contrast challenge", "conservative"], dangerous: true },
        ],
        explanation:
          "Closed loop obstruction with signs of ischemia needs surgery now. Conservative management suits simple adhesive obstruction only. Delay risks necrosis and perforation.",
        keyFeature: { topic: "abdominal-pain", n: 4 },
        source: "wses-sbo",
      },
      {
        id: "q4",
        kind: "short",
        update: "His lactate on arrival 3 hours earlier was 1.6 mmol/L.",
        prompt: "Why should that normal value not have been used to exclude ischemia?",
        required: 2,
        accept: [
          { id: "sens", text: "Lactate is insensitive in early ischemia", match: ["insensitive", "low sensitivity", "not sensitive", "early"] },
          { id: "liver", text: "The liver clears lactate from the portal circulation", match: ["liver", "hepatic", "clearance", "portal"] },
          { id: "trend", text: "It may rise later, so trends matter", match: ["serial", "repeat", "repeated", "trend", "rise", "later"] },
          { id: "clin", text: "Clinical and CT findings outweigh a single lab value", match: ["clinical", "ct finding", "imaging", "exam", "single"] },
        ],
        explanation:
          "No single laboratory test excludes bowel ischemia. Lactate often stays normal until ischemia is advanced. A closed loop can strangulate with little systemic signal.",
        keyFeature: { topic: "abdominal-pain", n: 6 },
        source: "wses-sbo",
      },
    ],
    sources: [SRC.rosen, SRC.wsesSbo],
    ...meta,
  },
];
