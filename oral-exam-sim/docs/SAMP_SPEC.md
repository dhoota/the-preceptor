...
- `match` phrases: lowercase. A candidate line matches when every word of any phrase appears in it, in any order. Plurals ending in s are ignored. So keep phrases short and specific: `match: ["ceftriaxone"]`, `match: ["ct angiogram", "cta"]`, `match: ["repeat troponin"]`. `match` is one flat list of strings. Add common abbreviations and generic class names the key accepts (`match: ["ppi", "pantoprazole", "proton pump inhibitor"]`). Avoid phrases so short they match wrong answers (never just `"iv"`).
- Negation: a phrase does not match when "no", "not", "avoid", "withhold", "hold", "never", "don't", "stop", "discontinue", "contraindicated" or "without" appears within three words before it. So "no nitrates" does not match `["nitrate"]`. When the correct answer is itself a negative ("Avoid nitrates", "Hold the heparin", "No imaging needed"), put the negation in the phrase: `match: ["avoid nitrate", "no nitrate", "hold nitrate"]`. Words that follow a phrase also negate it: "contraindicated", "avoided" or "withheld" right after it, or after "is" or "are" ("warfarin is contraindicated"). A negation within two words before a later word of the phrase also counts ("epinephrine IM, not IV push"). Apostrophes are dropped, so "don't" reads as "dont". The validator checks every key item matches its own text.
- Frequency: no key feature should be the main focus of more than 8 SAMPs in a topic. Individual questions may test a key feature more often.
